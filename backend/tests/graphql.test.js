...
describe("when there is event", () => {
  ...
  test("can vote event", async() => {
    const eventInDB = await helper.eventInDB()
    await helper.login(setOptions, mutate, helper.userObject.username, "salainen")
    const result = await mutate(VOTE_EVENT, { variables: { id: eventInDB._id.toString(), votes: [{ date: new Date(), vote: "red" }] } })
...
describe("when event has already been voted on", () => {
  beforeAll(async () => {
    ...
    const user = await helper.createUser()
    const group = await helper.createGroup([user])
    const event = new Event({ name: helper.eventObject.name, group: group, dates: [{ date: new Date(), votes: [{ voter: helper.userObject.username, vote: "red" }] }], status: "picking" })
    await event.save()
  })
  test("can change vote", async () => {
    ...
    await helper.login(setOptions, mutate, helper.userObject.username, "salainen")
    const result = await mutate(VOTE_EVENT, { variables: { id: eventInDB._id.toString(), votes: [{ date: new Date(), vote: "blue" }] } })
    console.log(result.data.voteEvent.dates[0].votes)
    const eventInDBBack = await helper.eventInDB()
    expect(eventInDBBack.dates[0].votes[0].voter).toStrictEqual(helper.userObject.username)
...
describe("multiple voters", () => {
  beforeEach(async () => {
    ...
    const event = Event({ name: helper.eventObject.name, group: group, dates: [{ date: new Date(), votes: [{ voter: helper.userObject.username, vote: "red" }] }, { date: new Date(), votes: [{ voter: helper.userObject.username, vote: "red" }] }], status: "picking" })
    await event.save()
  })
  ...
  test("algorithm picks correct finalDate", async () => {
    ...
    console.log(eventInDB)
    await helper.login(setOptions, mutate, helper.secondUserObject.username, "salainen")
    const result = await mutate(VOTE_EVENT, { variables: { id: eventInDB._id.toString(), votes: [{ date: new Date().toISOString(), vote: "green" }, { date: new Date().toISOString, vote: "red" }] } })
